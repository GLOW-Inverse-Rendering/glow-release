...
class ValidationHook:
    def __init__(self, cfg: ValidationConfig, train_render: RenderingConfig, scene: mi.Scene, integrator_injection: dict[str, Any], kwargs_injection: dict[str, Any]):
        ...
        train_render = OmegaConf.to_container(train_render, resolve=True)
        val_render = OmegaConf.to_container(cfg.rendering, resolve=True)
        ...
        self.integrator = create_integrator(self.rendering, scene, {"samples_per_pass": 1}, integrator_injection, kwargs_injection)
        if (self.use_hq):
            block_size = 64
            self.integrator = HighQuality(block_size, self.integrator)
            logger.info(f"High quality renderer being used for integrator {self.integrator} at block size: {block_size}")
