...
from .path import MyPathTracer, MyPathTracerMixin
class NeradMixin:
    ...
    def sample(self,
        ...
        with dr.suspend_grad(when=RHS_eval_doesnt_need_grad):
            with torch.set_grad_enabled(not RHS_eval_doesnt_need_grad):
                net_ins = self.extract_inputs(si, scene, sampler)
                RHS_net = dr.select(active & si.is_valid(), self.network.eval(*net_ins), mi.Vector3f(0))
        ...
        RHS_for_residual = stop_grad_radiance(RHS_net) * stop_grad_non_radiance(throughput) + stop_grad_non_radiance(bsdf_sample_result + em_sample_result)
    ...
    def extract_inputs(self, si, scene, sampler):
        pts = si.p
        ...
        if 'flashlight.position' in params.keys():
            ...
            point_light_dir = rot_transform @ mi.Vector3f([0.0, 0.0, 1.0]) + si.p*0
            # print(point_light_pos)
            # print(point_light_dir)

        return pts, dirs, normals, albedo, point_light_pos, point_light_dir, em_weight


@register_integrator("nerad")
class Nerad(NeradMixin, MyPathTracer, nn.Module):
    def __init__(self, props: mi.Properties):
        ...
        self.detach_radiance_gradients_in_residual = props.get("config").dict.get("detach_radiance_gradients_in_residual")
        self.detach_non_radiance_gradients_in_residual = props.get("config").dict.get("detach_non_radiance_gradients_in_residual")
