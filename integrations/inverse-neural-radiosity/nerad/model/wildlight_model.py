...
import functorch
import numpy as np
from .wildlight_embedder import get_embedder, Embedder
import tinycudann as tcnn
import math

class NeuralSDF():
    def sdf_unsqueeze(self, x):
        return self(x.unsqueeze(dim=0))[0, 0]

    def sdf(self, x):
        y= self.forward(x)[:, :1]
        # print("sdf requires_grad", y.requires_grad)
        return y
...
class SDFNetwork(NeuralSDF, nn.Module):
    def __init__(self,
                 d_in,
                 d_out,
                 d_hidden,
...
class TCNNMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.geometric_init:
            ...
            return result
        # return result
class SDFNetworkNGP(NeuralSDF, nn.Module):
    def __init__(self,
                 d_in,
                 d_out,
                 d_hidden,
...
class RenderingNetworkIRON(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors):
        ...
        # print("view_dirs.shape", view_dirs.shape)
        if self.mode == "idr":
            rendering_input = torch.cat([points, view_dirs, normals, feature_vectors], dim=-1)
        elif self.mode == "no_view_dir":
            rendering_input = torch.cat([points, normals, feature_vectors], dim=-1)
