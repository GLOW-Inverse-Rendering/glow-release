...
class TransferMLP(nn.Module):
    ...
    def forward(self, points, dirs, normals, albedo, points2):
        net_in = torch.cat(
            [
...
@wrapper_registry.register("transfer_net")
class MitsubaTransferNetworkWrapper(MitsubaWrapper):
    def __init__(
        ...
        self.network = TransferMLP(width, hidden, position1_embedding, direction_embedding, position2_embedding , scene_properties_input, no_albedo)
    ...
    def eval_torch(self, pts, dirs, norms, albedo, p2_tensor, active):
        ...
        return self.network(pts, dirs, norms, albedo, p2_tensor)
