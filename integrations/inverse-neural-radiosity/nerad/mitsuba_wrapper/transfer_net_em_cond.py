...
class TransferMLP(nn.Module):
    def __init__(
        ...
        in_size = embed_size(3, self.pos_emb) + embed_size(3, self.dir_emb) + embed_size(3, self.pos2_emb)
    ...
    def forward(self, points, dirs, normals, albedo, points2, em_weight):
        net_in = torch.cat(
            [
...
@wrapper_registry.register("transfer_net_em_cond")
class MitsubaTransferNetworkWrapper(MitsubaWrapper):
    def __init__(
        ...
        self.network = TransferMLP(width, hidden, position1_embedding, direction_embedding, position2_embedding , scene_properties_input, no_albedo, em_cond)
    ...
    def _eval(self, pts, dirs, norms, albedo, pts2, dirs2, em_weight, active):
        ...
        torch_out = self.eval_torch(
            p_tensor, d_tensor, n_tensor, alb_tensor, p2_tensor, em_tensor, active)
        dr.make_opaque(torch_out)
        output = dr.unravel(mi.Vector3f, torch_out.array)
        return dr.abs(output)

    @dr.wrap_ad(source='drjit', target='torch')
    def eval_torch(self, pts, dirs, norms, albedo, p2_tensor, em_tensor, active):
        ...
        return self.network(pts, dirs, norms, albedo, p2_tensor, em_tensor)
